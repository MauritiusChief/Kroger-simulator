...
for (const item of inventoryEntities) {
  trends7Seed[item.id] = makeTrend(7, Math.floor(item.sales7d / 7)).map((point) => ({
    d: point.date,
    units: point.value
  }));
  trends30Seed[item.id] = makeTrend(30, Math.floor(item.sales30d / 30)).map((point) => ({
    d: point.date,
    units: point.value
  }));
}
